...
export class HttpService {
  ...
  async post<T, U = any>(url: string, data: U, options?: RequestInit): Promise<T> {
    try {
      const response = await fetch(url, {
        method: 'POST',
        headers: {
  ...
  async put<T, U = any>(url: string, data: U, options?: RequestInit): Promise<T> {
    try {
      const response = await fetch(url, {
        method: 'PUT',
        headers: {
  ...
  /**
   * Safely extract data from API response with success/error structure
   */
